...
test.describe("Smoke prod (lecture seule)", () => {
  test("homepage utilise l'API prod et rend les listes", async ({ page }) => {
    ...
    await expect(page.getByTestId("momentum-list")).toBeVisible({ timeout: 20_000 });
    await expect(page.getByTestId("planning-list")).toBeVisible({ timeout: 20_000 });
  });
  ...
  test("navigation vers un détail match et appels non-localhost", async ({ page }) => {
    ...
    await firstMatch.waitFor({ state: "visible", timeout: 20_000 });
    ...
    await expect(page.getByTestId("match-score")).toBeVisible({ timeout: 20_000 });
    await expect(page.getByTestId("classement-section")).toBeVisible({ timeout: 20_000 });
  });
});
