...
export function requireBaseUrl() {
    ...
    if (!envBaseUrl || envBaseUrl.trim().length === 0) {
        const fallback = typeof window !== "undefined" && window.location
            ...
            : undefined;
        if (fallback) {
            console.warn("VITE_API_BASE_URL manquante : utilisation du fallback window.location.origin =", fallback);
            return fallback;
        }
        console.error("VITE_API_BASE_URL is missing; available env keys =", env ? Object.keys(env) : "none");
        throw new Error("VITE_API_BASE_URL manquante : definis-la dans .env.local (ex: http://localhost:3000).");
    }
