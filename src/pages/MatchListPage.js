...
export default function MatchListPage({ searchQuery = "", sort, onSortChange, }) {
    ...
    const momentumMatches = React.useMemo(() => {
        if (!filteredMatches || filteredMatches.length === 0)
            return [];
        const sortByDateAsc = [...filteredMatches].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    const renderLeading = (item) => (_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(HexBadge, { name: item.teamA, imageUrl: item.teamALogo ?? undefined, size: 44 }), _jsx("span", { className: "text-xs uppercase text-slate-500", children: "vs" }), _jsx(HexBadge, { name: item.teamB, imageUrl: item.teamBLogo ?? undefined, size: 44 })] }));
    const sortOptions = [
        { label: "Date", key: "date" },
