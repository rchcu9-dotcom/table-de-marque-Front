...
export default function PlanningPage() {
  ...
  const [surfaceFilter, setSurfaceFilter] = React.useState<Set<string>>(new Set(["GG"]));
  ...
  const filtered = React.useMemo(() => {
    ...
    return [...matches]
      .filter((m) => {
        ...
        if (surfaceFilter.size > 0 && !surfaceFilter.has(m.surface ?? "")) return false;
  ...
  }, [matches, competitionFilter, surfaceFilter, dayFilter, teamFilter, search]);
  ...
  React.useEffect(() => {
    ...
    const ongoing = filtered.find(
      (m) => m.competitionType === "5v5" && m.status === "ongoing",
    );
      ...
      >
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400">Compétition</label>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400">Glace</label>
              <div className="flex flex-wrap gap-2">
                {["GG", "PG"].map((g) => {
                  const active = surfaceFilter.has(g);
                  return (
                    <button
                      key={g}
                      type="button"
                      onClick={() => {
                        const next = new Set(surfaceFilter);
                        if (next.has(g)) {
                          next.delete(g);
                        } else {
                          next.add(g);
