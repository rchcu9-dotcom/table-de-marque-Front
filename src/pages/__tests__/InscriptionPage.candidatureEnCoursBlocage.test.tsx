...
const MESSAGE_DEJA_EN_COURS = "L'inscription de cette équipe a déjà été demandée.";
...
describe("InscriptionPage — blocage anticipé équipe déjà en cours d'inscription", () => {
  it("affiche le message de blocage et masque le bouton « Lancer la demande » pour une équipe avec candidatureEnCours: true", async () => {
    ...
    await screen.findByText(MESSAGE_DEJA_EN_COURS, { exact: false });
    expect(screen.queryByRole("button", { name: /Lancer la demande/ })).not.toBeInTheDocument();
    expect(mockSoumettreCanditature).not.toHaveBeenCalled();
  });

  it("affiche normalement le bouton « Lancer la demande » pour une équipe avec candidatureEnCours: false", async () => {
    mockFetchEquipesReferentiel.mockResolvedValue([
      { id: 2, nom: "Nouvelle Équipe", logoUrl: undefined, active: true, candidatureEnCours: false },
    ]);
    ...
    expect(screen.queryByText(MESSAGE_DEJA_EN_COURS, { exact: false })).not.toBeInTheDocument();
  });
});
