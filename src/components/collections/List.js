...
export default function List({ items, fields, sort, onItemClick, renderLeading, alignCenter, }) {
    ...
    return (_jsx("div", { className: `space-y-3 ${alignCenter ? "flex flex-col items-center w-full" : ""}`, children: sortedItems.map((item, idx) => {
            ...
            return (_jsx(Card, { className: `${onItemClick ? "cursor-pointer hover:bg-slate-800/80" : ""} ${alignCenter ? "flex flex-col items-center text-center w-full" : ""}`, onClick: () => onItemClick?.(item), children: _jsxs("div", { className: `flex w-full flex-col gap-1 ${alignCenter ? "items-center text-center" : ""}`, children: [renderLeading && (_jsx("div", { className: `mb-2 flex items-center ${alignCenter ? "w-full justify-center" : ""}`, children: renderLeading(item) })), primary && (_jsx("div", { className: `text-sm font-semibold ${alignCenter ? "w-full text-center" : ""}`, children: primary.render
